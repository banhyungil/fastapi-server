...
class ProcessStepBase(CamelModel):
    ...
    preset_id: int | None = Field(None, description="참조 프리셋 ID")
    parent_id: int | None = Field(None, description="부모 노드 ID (NULL이면 루트)")
...
class ProcessStepResponse(ProcessStepBase):
    ...
    process_id: int = Field(..., description="소속 프로세스 ID")
...
class ProcessCreate(CamelModel):
    ...
    file_id: int = Field(..., description="원본 파일 ID")
...
class ProcessUpdate(CamelModel):
    ...
    final_file_id: int | None = Field(None, description="최종 결과 파일 ID")
