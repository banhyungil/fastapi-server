...
from app.services import processes_service as svc
...
async def list_process(
    ...
    """처리연산 목록 조회"""
    items = svc.list_processes(file_id=file_id)
...
async def get_process(process_id: int) -> ProcessResponse:
    """처리연산 상세 조회"""
...
async def create_process(body: ProcessCreate) -> ProcessResponse:
    """처리연산 생성"""
    steps = [step.model_dump() for step in body.steps]
    result = svc.create_process(
        nm=body.nm,
...
async def update_process(process_id: int, body: ProcessUpdate) -> ProcessResponse:
    """처리연산 수정"""
    steps = [step.model_dump() for step in body.steps] if body.steps is not None else None
    result = svc.modify_process(
        process_id,
        nm=body.nm,
...
async def delete_process(process_id: int) -> None:
    """처리연산 삭제"""
    if not svc.remove_process(process_id):
        raise HTTPException(status_code=404, detail="process not found")
